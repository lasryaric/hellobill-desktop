...
const Bouygues = require('./connectors/bouygues');
const Facebookads = require('./connectors/facebookads');
const Dropbox = require('./connectors/dropbox');
const Ovh = require('./connectors/ovh');
const Slack = require('./connectors/slack');
const Orange = require('./connectors/orange');
...
function createBrowserWindow(mConnector, callback) {
  ...
  var theWindow = new BrowserWindow({width: 1024, height: 860, preload: preload, nodeIntegration: false, show:showCrawler,
    ...
    theWindow.loadURL('https://www.google.com');
  ...
  function runIt(mUserMe, config, date) {
    return new bluebird.Promise(function(resolve, reject) {
      ...
      .then((bw) => {
        ...
        const oneRunner = new BrowserAutomation(bw, config.name, mUserMe.destinationFolder, mUserMe.email, credentials.username, config);
