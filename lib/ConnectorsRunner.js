...
function createBrowserWindow(mConnector, callback) {
  ...
  var theWindow = new BrowserWindow({width: 1024, height: 860, show:showCrawler,
    ...
    theWindow.on('closed', function() {
      ...
      // theWindow.webContents.removeAllListeners();
      // theWindow.removeAllListeners();
    ...
    //   winston.info('2. did-finish-load for %s', ax.sender.getURL());
    //
  ...
  function closeBrowserWindow(mConnector) {
    return new Promise(function (yes, no) {
      ...
      connectorTransitions[mConnector.name] = connectorName2BrowserWindow[mConnector.name];
      setTimeout(() => {
        connectorTransitions[mConnector.name].close();
        connectorTransitions[mConnector.name] = null;
        delete(connectorTransitions[mConnector.name]);
      ...
      connectorName2BrowserWindow[mConnector.name].hide();
    ...
    return new bluebird.Promise(function(resolve, reject) {
      ...
      .then((bw) => {
        ...
        .finally(() => {
          ...
          oneRunner.getBrowserWindow().loadURL('http://localhost:3000/desktop/bootstrap');
          oneRunner.cleanup();
          winston.info('done cleanup %s', config.name)
