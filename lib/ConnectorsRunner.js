...
function createBrowserWindow(mConnector, callback) {
  ...
  var theWindow = new BrowserWindow({width: 1024, height: 860, preload: preload, nodeIntegration: false, show:true,
    ...
    theWindow.webContents.on('did-finish-load', function(ax) {
      ...
      winston.info('starting __hellobill.runLoop()', ax.sender.getURL());
  ...
  function runIt(mUserMe, config, date, callback) {
    ...
    .then((bw) => {
      ...
      oneRunner.emitter.on('error', (err) => {
        ...
        winston.error('calling error method 1', config.name, err)
        onDoneOnce(err)
      })
      ...
      .catch(function(err) {
        ...
        // bw.close();
        // bw = null;
        winston.error('calling error method 2', config.name, err)
        onDoneOnce(err);
  ...
  function runThem(mUserMe, list, date, callback) {
    ...
    if (config.error && config.error.errorName === 'ConnectorErrorWrongCredentials') {
      winston.error('we know we have a wrong login / password for this connector: %s', config.name);
