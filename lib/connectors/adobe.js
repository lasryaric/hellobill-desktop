...
function Connector() {
  ...
  this.run = function(date, credentials, oneRunner, callback) {
    ...
    .then(() => {
      console.log('Waiting one second now...')
      return oneRunner.waitOnCurrentThreadAsync(1000);
    })
    .then(() => {
      console.log('DONE WAITING second now...')
    ...
    .then((elementExists) => {
      ...
      if (!elementExists.elementExists) {
        ...
        .then(() => {
          return oneRunner.waitOnCurrentThreadAsync(1)
        })
        .then(() => {
    ...
    .then(() => {
      return oneRunner.waitForCssAsync('#adobeid_username')
    })
    .then((elementExists) => {
      if(elementExists.elementExists) {
          return oneRunner
