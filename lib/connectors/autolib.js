    ...
    return new bluebird.Promise((resolve, reject) => {
      ...
      .then((ex) => {
        if (ex.ex.login) {
          ...
          .then(() => {
            return oneRunner.waitForCssAsync({user:'div#div_username input#id_username'})
          })
          .then(() => {
            return oneRunner.typeTextAsync('div#div_username input#id_username', credentials.username)
          })
          .then(() => {
            return oneRunner.typeTextAsync('div#div_password input#id_password', credentials.password)
          })
          .then(() => {
            return oneRunner.waitForCssAsync({button:'button[type="submit"][value="Log in"]',input:'input[type="submit"][value="Log in"]'})
          })
          .then((ex) => {
            if (ex.ex.input) {
              return oneRunner.clickAsync('input[type="submit"][value="Log in"]')
            } else {
              return oneRunner.clickAsync('button[type="submit"][value="Log in"]')
            }
          })
        }
      })
