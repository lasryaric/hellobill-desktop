    ...
    return new bluebird.Promise((resolve, reject) => {
      ...
      .then((ex) => {
        if (ex.ex.login) {
          ...
          .then(() => {
            return oneRunner.elementExistsAsync('#lastname');
          })
          .then((elementExists) => {
            if (elementExists.elementExists) {
              return oneRunner.typeTextAsync('#lastname', credentials.lastname);
            }
          })
          .then(() => {
      ...
      .then(() => {
        return oneRunner.validateLoginAsync({login:'#log_data', billing:'a[href="/mon-compte/mes-factures"]'}, {})
      })
      .then((ex) => {
        if (ex.ex.login) {
