    ...
    return new bluebird.Promise((resolve, reject) => {
      ...
      .each((accountURL) => {
        ...
        if (urlProps && urlProps.query && urlProps.query.act) {
          ...
          const invoicesURL = 'https://www.facebook.com/ads/manager/billing/transactions_by_page/?act='+act+'&pid=p2&date='+dateRangeStr;
          ...
          .then(() => {
            return oneRunner.clientSideFunctionAsync('facebookads',{},'getInvoicesURLS')
            .each((result) => {
              return oneRunner.clientSideFunctionAsync('facebookads',{urls:result[1]},'pageUrls')
              .each((url) => {
                return oneRunner.gotoAsync(url)
                .then(() => {
                  return oneRunner.waitForCssAsync({down:'a[href*="manage/billing_transaction.php"]'})
                })
                .then(() => {
                  return oneRunner.waitForDownloadAsync('facebookads', date, result[0])
                })
              })
            })
          })
