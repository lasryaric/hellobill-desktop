...
function Connector() {
  ...
  this.run = function(date, credentials, oneRunner) {
    return new bluebird.Promise((resolve, reject) => {
      ...
      .then(() => {
        return oneRunner.gotoAsync('https://admin.google.com/')
      })
      .then(() => {
        return oneRunner.waitForCssAsync({bil:'img[src*="img/billing"]'})
      })
      .then(() => {
      ...
      .then(() => {
        return oneRunner.waitForCssAsync({pop:'.popupContent div[role="menu"] div'})
      })
      .then(() => {
...
function logMeIn() {
  this.run = function(date, credentials, oneRunner) {
    ...
    return new bluebird.Promise((resolve, reject) => {
      ...
      .then(() => {
        return oneRunner.validateLoginAsync({wrongPassword:'#errormsg_0_Passwd', ok:'a[href*="SignOutOptions"]'}, {'twostep':'form#challenge'})
      })
      .then((ex) => {
