...
function Connector() {
  ...
  this.run = function(date, credentials, oneRunner) {
    return new bluebird.Promise((resolve, reject) => {
      ...
      .then(() => {
        return oneRunner.gotoAsync('https://www.ovh.com/manager/web/#/billing/history');
      })
      .then(() => {
...
function logMeIn() {
  this.run = function(date, credentials, oneRunner) {
    ...
    return new bluebird.Promise((resolve, reject) => {
      ...
      .then(() => {
        return oneRunner.waitForCssAsync({login:'input[type="password"]', ok:'.wreckme.pull-left',panel:'button[value="gotomanager"]'})
      })
      .then((ex) => {
        if (ex.ex.login) {
          return oneRunner
      ...
      .then(() => {
        return oneRunner.validateLoginAsync({login:'input[type="password"]', ok:'.wreckme.pull-left',panel:'button[value="gotomanager"]'}, null)
      })
      .then((ex) => {
        if (ex.ex.login) {
