...
function SkypeConnector(bw) {
  ...
  this.run = function(date, callback) {
    ...
    .then(() => {
      const url = 'https://secure.skype.com/wallet/account/orders?month='+date;

      return oneRunner.gotoAsync(url)
    })
    .then(() => {
      const fileName = "skype-"+ date +".pdf";

      return oneRunner.clickAndWaitForDownloadAsync('#downloadMonthlyStatement a', fileName)
    })
    .then(() => {
      callback();
    })
