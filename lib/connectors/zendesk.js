...
function Connector() {
  ...
  this.run = function(date, credentials, oneRunner) {
    return new bluebird.Promise((resolve, reject) => {
      ...
      .then(() => {
        return oneRunner.gotoAsync('https://'+credentials.compname+'.zendesk.com/settings/account');
      })
      .then(() => {
        return oneRunner.waitForCssAsync({tab:'a[data-title-value="Invoices"]'})
      })
      .then(() => {
        return oneRunner.clickAsync('a[data-title-value="Invoices"]')
      })
      .then(() => {
